...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({ initData }) => {
  ...
  const myMatches = matches.filter(
    (m) => m.userA.id === user?.id || m.userB.id === user?.id,
  );
      ...
      {myMatches.length > 0 ? (
        <>
          <div style={styles.matchBanner}>
            ...
            <h3 style={styles.matchTitle}>У тебя совпадение!</h3>
          </div>
          ...
          {myMatches.map((match) => {
            ...
            return (
              <div key={match.id} style={styles.matchCard}>
                ...
                <a
                  href={getContactLink(partner.telegramId, partner.username)}
                  target="_blank"
        ...
        <div style={styles.noMatch}>
          ...
          <p style={styles.noMatchText}>
            В этот раз совпадений не случилось. Попробуй ещё раз — в следующий раз повезёт!
          </p>
        </div>
      )}
        ...
        <div style={styles.otherMatches}>
          ...
          {otherMatches.map((m) => (
            <div key={m.id} style={styles.otherMatchRow}>
              ...
              <span style={{ fontSize: 12 }}>{m.userA.firstName}</span>
              <span style={{ color: '#FF6B6B' }}>♥</span>
              <span style={{ fontSize: 12 }}>{m.userB.firstName}</span>
